...
def find_best_junctions(
    ...
    """
    Return the 3 biggest junctions multiplied together.
    """
    ...
    for (box_a, box_b), dist in sorted_distances[:max_connections]:
        ...
        if circuit_a != circuit_b:
            ...
            print([c.total_junctions() for c in circuits])

    largest_junctions = sorted([c.total_junctions() for c in circuits], reverse=True)[:3]
    return largest_junctions[0] * largest_junctions[1] * largest_junctions[2]

if __name__ == "__main__":
    filename = "input.txt"
    print(find_best_junctions(filename, max_connections=1000))
