# Advent of Code — Solutions

This repository contains my solutions for Advent of Code challenges. Each day has a small folder with the puzzle statement, input files and one or two Python solution scripts.

**How to use**
- Each day is in a folder named `day-XX-Name/`.
- Puzzle inputs are in `input.txt` and `input-test.txt` inside each day folder.
- Solutions are in `solution-01.py` and `solution-02.py` (part 1 and part 2 respectively).
- Run a solution with Python, for example:

```
python day-01-SecretEntrance/solution-01.py
```

**Answers**

| Day | Name | Part 1 | Part 2 |
|---:|---|---:|---:|
| 1 | SecretEntrance | <details><summary>•••</summary><code>1055</code></details> | <details><summary>•••</summary><code>6386</code></details> |
| 2 | GiftShop | <details><summary>•••</summary><code>23701357374</code></details> | <details><summary>•••</summary><code>34284458938</code></details> |
| 3 | Lobby | <details><summary>•••</summary><code>17554</code></details> | <details><summary>•••</summary><code>175053592950232</code></details> |
| 4 | PrintingDepartment | <details><summary>•••</summary><code>1569</code></details> | <details><summary>•••</summary><code>9280</code></details> |
| 5 | Cafeteria | <details><summary>•••</summary><code>664</code></details> | <details><summary>•••</summary><code>350780324308385</code></details> |
| 6 | TrashCompactor | <details><summary>•••</summary><code>5733696195703</code></details> | <details><summary>•••</summary><code>10951882745757</code></details> |
| 7 | Laboratories | <details><summary>•••</summary><code>1646</code></details> | <details><summary>•••</summary><code>32451134474991</code></details> |

**Notes**
- The solutions are plain Python scripts that read `input.txt` from their folder by default. If you change the input file location, update the script call accordingly.